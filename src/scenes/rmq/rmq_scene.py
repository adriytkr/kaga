...
class RMQScene(RMQSceneAssets):
  def construct(self):
    ...
    for el in self.array_vgroup.target:
      el[2].next_to(el[0],LEFT,buff=0.3)
    ...
    table_group.next_to(self.array_vgroup,RIGHT,buff=1.6)

    for row_idx in range(self.ROW_COUNT):
      self.st_grid[row_idx].align_to(
        self.array_vgroup[row_idx][0],UP
      )
      row_labels[row_idx].next_to(
        self.st_grid[row_idx],LEFT,buff=0.25
      )

    for j,lbl in enumerate(col_labels):
      lbl.next_to(self.st_grid[0][j],UP,buff=0.25)
    ...
    self.play(LaggedStart(*animations,lag_ratio=0.15),run_time=1.5)
